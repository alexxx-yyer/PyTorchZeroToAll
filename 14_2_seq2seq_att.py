...
from seq2seq_models import cuda_tensor, str2tensor, EOS_token, SOS_token
...
def test():
    ...
    word_input = cuda_tensor(torch.tensor([1, 2, 3], dtype=torch.long))
    encoder_outputs, encoder_hidden = encoder_test(word_input, encoder_hidden)
    print(encoder_outputs.size())

    word_target = cuda_tensor(torch.tensor([1, 2, 3], dtype=torch.long))
    ...
    for c in range(len(word_target)):
        ...
        decoder_attns[0, c] = decoder_attn.squeeze(0).detach().cpu()
...
def translate(enc_input='thisissungkim.iloveyou.', predict_len=100, temperature=0.9):
    ...
    for c in range(predict_len):
        ...
        attentions.append(attention.view(-1).detach().cpu().numpy().tolist())
